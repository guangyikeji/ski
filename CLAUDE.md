...
质量系数：基于参赛选手中最好5名的平均积分计算
人数系数：根据参赛人数调整（30人以下0.9，30-60人1.0，60人以上1.1）
附加分：地域鼓励分、进步奖励分等
```
...
#### 俱乐部积分评价体系 ⭐ 全新模块
```javascript
const CLUB_STAR_RATINGS = {
  FIVE_STAR: { min: 800, benefits: '顶级培训资源、优先参赛权' },
  FOUR_STAR: { min: 600, benefits: '高级培训支持、赛事优惠' },
  THREE_STAR: { min: 400, benefits: '标准培训资源、正常参赛' },
  TWO_STAR: { min: 200, benefits: '基础培训支持、限制参赛' },
  ONE_STAR: { min: 0, benefits: '最低标准、观察期' }
};

// 俱乐部积分 = 运动员贡献分 + 培训贡献分 + 赛事组织分
```
...
#### 必须实现的核心功能 ⭐ v2.0升级
1. **多重系数积分引擎**: 支持赛事级别、质量、人数等多维系数计算
2. **分层青少年管理**: U10-U21五个年龄组的差异化积分算法
3. **俱乐部评价系统**: 五星级评定制度和贡献分计算
4. **赛事分级管理**: A/B/C/D四级赛事系数体系
5. **24个月滚动机制**: 积分有效期管理和权重衰减
6. **数字化功能**: APP查询、AI分析、区块链认证
7. **运动员全生命周期**: 从注册到退役的完整管理
8. **伤病保护升级**: 多元化保护模式和智能申请流程
