/**
 * 自由式滑雪积分系统 (Freestyle Ranking Points System)
 * 特点：积分越高越好，赛季取最好成绩累计
...
// 自由式滑雪项目枚举
export enum FreestyleDiscipline {
  BA = 'BA',    // 大跳台 (Big Air)
...
// 自由式比赛数据接口
export interface FreestyleRaceData {
  athleteId: string;
  discipline: FreestyleDiscipline;
  ...
  performanceDetails?: {
    ...
    amplitude?: number;            // 腾空高度（U型场地专用）
    variety?: number;              // 动作多样性
  };
...
// 自由式赛季数据接口
export interface FreestyleSeasonData {
  athleteId: string;
  discipline: FreestyleDiscipline;
  season: string;              // 格式: "2025-2026"
  baselinePoints?: number;     // 基础积分（上赛季延续）
  races: FreestyleRaceData[];
  seasonBestCount?: number;    // 取最好几次成绩（默认5次）
}

// 自由式积分计算结果
export interface FreestylePointsResult extends UnifiedPointsResult {
...
export class FreestyleRankingPointsSystem {
   ...
   * 计算单场比赛积分
   */
  calculateRacePoints(raceData: FreestyleRaceData): FreestylePointsResult {
    // 验证数据
    ...
    return {
      ...
      discipline: `FREESTYLE_${raceData.discipline}`,
      systemType: ScoringSystemType.FREESTYLE_RANKING_POINTS,
   ...
   * 特点：取最好成绩累计（高分制）
   */
  calculateSeasonPoints(seasonData: FreestyleSeasonData): SeasonPointsSummary {
    // 计算所有比赛的积分
    ...
    return {
      ...
      discipline: `FREESTYLE_${seasonData.discipline}`,
      systemType: ScoringSystemType.FREESTYLE_RANKING_POINTS,
   ...
   * 批量计算多个运动员的比赛积分
   */
  calculateBatchRacePoints(raceDataList: FreestyleRaceData[]): FreestylePointsResult[] {
  ...
  calculateRaceRanking(
    raceDataList: FreestyleRaceData[],
    addRankInfo: boolean = true
  ): FreestylePointsResult[] {
    // 按得分排序确定排名（得分越高排名越靠前）
   ...
   */
  analyzePointsTrend(
    seasonData: FreestyleSeasonData
  ): {
    ...
    if (points.length < 2) {
      return {
        ...
        medalCount: {
          gold: ranks.filter(r => r === 1).length,
          silver: ranks.filter(r => r === 2).length,
          bronze: ranks.filter(r => r === 3).length
        }
      };
    }
  ...
  generatePointsReport(
    ...
    trend: ReturnType<FreestyleRankingPointsSystem['analyzePointsTrend']>;
    recommendations: string[];
    disciplineSpecificAdvice: string[];
  } {
    const summary = this.calculateSeasonPoints(seasonData);
    const trend = this.analyzePointsTrend(seasonData);

    const recommendations: string[] = [];
    const disciplineSpecificAdvice: string[] = [];

    if (trend.trend === 'improving') {
      recommendations.push('自由式滑雪积分持续上升，保持当前训练强度');
    ...
    if (trend.medalCount.gold === 0 && summary.participatedEvents > 3) {
      recommendations.push('缺少冠军表现，建议提高难度动作和完成质量');
    }

    // 项目特定建议
    switch (seasonData.discipline) {
      case FreestyleDiscipline.BA:
        disciplineSpecificAdvice.push('大跳台项目：专注于起跳时机和空中动作控制');
        if (trend.consistency < 0.7) {
          disciplineSpecificAdvice.push('建议加强空中意识训练和落地技术');
        }
        break;

      case FreestyleDiscipline.SS:
        disciplineSpecificAdvice.push('坡面障碍技巧：注重线路选择和道具连接流畅性');
        if (trend.topRankings < 2) {
          disciplineSpecificAdvice.push('建议增加道具训练时间，提高创意性');
        }
        break;

      case FreestyleDiscipline.HP:
        disciplineSpecificAdvice.push('U型场地：平衡腾空高度和动作难度');
        if (trend.averageChange < 0) {
          disciplineSpecificAdvice.push('建议加强壁面利用和节奏控制训练');
        }
        break;
    }
  ...
  analyzeFreestyleSpecifics(
    ...
    competitivePosition: {
      strongCompetitions: string[];
      improvementAreas: string[];
    };
  } {
    const results = seasonData.races.map(race => this.calculateRacePoints(race));
    const scores = seasonData.races
      .filter(race => race.finalScore !== undefined)
      .map(race => race.finalScore!);
    ...
    // 计算改进率（基于积分变化）
    const points = results.map(r => r.points.earnedPoints);
    const improvementRate = points.length > 1 ?
      (points[points.length - 1] - points[0]) / points[0] : 0;

    // 技术分析（基于可用数据）
    const consistencyScores = scores.length > 1 ?
      1 - (Math.sqrt(scores.reduce((sum, s) => sum + Math.pow(s - averageScore, 2), 0) / scores.length) / averageScore) : 1;

    const strongCompetitions = seasonData.races
      .filter(race => race.finalRank <= 3)
      .map(race => race.competitionId);

    const improvementAreas = [];
    if (results.filter(r => r.rank <= 8).length / results.length < 0.5) {
      improvementAreas.push('决赛晋级率');
    }
    if (results.filter(r => r.rank <= 3).length === 0) {
      improvementAreas.push('冲击奖牌能力');
    }
    if (consistencyScores < 0.7) {
      improvementAreas.push('成绩稳定性');
    }
    ...
    return {
      ...
      technicalAnalysis: {
        difficultyTrend: improvementRate > 0.1 ? 'increasing' : improvementRate < -0.1 ? 'decreasing' : 'stable',
        executionConsistency: Math.round(consistencyScores * 100) / 100,
        riskRewardBalance: Math.round((results.filter(r => r.rank <= 8).length / results.length) * 100) / 100
      },
      competitivePosition: {
        strongCompetitions,
        improvementAreas
  ...
  private validateRaceData(raceData: FreestyleRaceData): void {
    ...
    if (!Object.values(FreestyleDiscipline).includes(raceData.discipline)) {
      throw new Error('无效的自由式滑雪项目');
    }
...
// 导出单例实例
export const freestyleRankingPointsSystem = new FreestyleRankingPointsSystem();

// 导出便捷工具函数
export const FreestyleRankingPointsUtils = {
  /**
   * 快速计算积分
   */
  quickCalculate: (
    ...
    return freestyleRankingPointsSystem.calculatePointsByRank(rank, pointsCategory);
  },

  /**
   * 获取所有自由式滑雪项目
   */
  getAllDisciplines: () => Object.values(FreestyleDiscipline),

  /**
   * 获取项目中文名称
   */
  getDisciplineName: (discipline: FreestyleDiscipline) => {
    const names = {
      [FreestyleDiscipline.BA]: '自由式大跳台',
      [FreestyleDiscipline.SS]: '自由式坡面障碍技巧',
      [FreestyleDiscipline.HP]: '自由式U型场地'
    };
  ...
  calculateRankPointsDifference: (
    ...
    const points1 = freestyleRankingPointsSystem.calculatePointsByRank(rank1, pointsCategory);
    const points2 = freestyleRankingPointsSystem.calculatePointsByRank(rank2, pointsCategory);
  ...
  generateRankingTable: (
    ...
    for (let rank = 1; rank <= maxRank; rank++) {
      const points = freestyleRankingPointsSystem.calculatePointsByRank(rank, pointsCategory);
   ...
   * 计算项目间积分转换
   */
  convertPointsBetweenCategories: (
    points: number,
    fromCategory: PointsCategory,
    toCategory: PointsCategory
  ) => {
    const fromMax = POINTS_DISTRIBUTION[fromCategory].maxPoints;
    const toMax = POINTS_DISTRIBUTION[toCategory].maxPoints;

    const ratio = points / fromMax;
    const convertedPoints = ratio * toMax;
